...
const PinGeneration = ({ navigation }) => {
  ...
  const numericPad = numbers.map((v) =>
    v == 10 ? "close" : v == 11 ? "0" : v == 12 ? "" : v
  );
  ...
  const CheckPins = () => {
    if (code.length == 4) {
      if (code.length == confirmCode.length) {
        const check = code.every(
          (v, index) => parseInt(v) == parseInt(confirmCode[index])
        );
        if (check) {
        ...
        >
          {item !== "close" ? (
            item == "" ? (
              ...
              <Text
                style={[styles.padItemText, { fontSize: number_Size * 0.5 }]}
              >
                {item}
              </Text>
            )
  ...
  const renderHeading = () => {
    return (
      <MotiView
        style={[styles.successContainer, { flexDirection: "row" }]}
        from={{ translateY: 20, scaleX: 0 }}
        animate={{ translateY: 0, scaleX: 1 }}
      >
        <Text style={styles.headingText}>
          {code.length == 4
            ? "Confirm Your 4 Digits Pin"
  ...
  const renderTick = () => (
    <>
      ...
      >
        ...
        <MotiView
          style={[styles.successContainer, { flex: 0 }]}
    ...
    <View style={styles.container}>
      <View
        style={[
          ...
          { backgroundColor: "rgba(6, 3, 40,0.2)" },
        ]}
      />
      <TouchableOpacity
        onPress={() => {
